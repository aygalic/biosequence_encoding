...
# legacy copy
def generate_dataset_LEGACY(path = absolute_path, 
                     metadata_path = metadata_path,
                     feature_selection_threshold = None, 
                     batch_size = 64, 
                     subsample = None, 
    ...
    if(as_time_series == True):
        print("converting samples to time series")
        big_bad_dict = {key: [sample for (sample, name) in zip(data_array, entries_transcripts) if int(name.split(".")[1]) == key] for key in patient_id}
        ...
        print("keeping sample as is, no conversion to time series")
        x_train = tf.data.Dataset.from_tensor_slices(data_array)
        sequence_names = [f for (f, test) in  zip(entries_transcripts, samples_to_keep) if test]
